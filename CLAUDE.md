...
**톤 가이드** (방어적 disclaimer 금지): README와 사이트 모두에서 정책 명시는 차분한 1회 — 한국어 primary 자연스러운 한 줄(예: `토스와 제휴하지 않은 커뮤니티 프로젝트입니다.`). 헤더 직후의 `>` blockquote 박스, ⚠️ 아이콘, 굵은 글씨, `unofficial`/`비공식` 같은 강한 라벨, 영/한 병기는 모두 쓰지 않는다.
