...
- **devtools mock**: the [`@ait-co/devtools`](https://github.com/apps-in-toss-community/devtools) mock resolves `appLogin` immediately with a fake `authorizationCode`. Server-side token exchange must be mocked separately — only the shape is real, not the upstream behavior.
