...
- **Real Toss app**: `eventLog` calls are forwarded to the logging system. Entries may be routed into different pipelines depending on `log_type`.
- **devtools mock**: the mock injected by [`@ait-co/devtools`](https://github.com/apps-in-toss-community/devtools) prints `eventLog` calls to the **browser console**, letting you inspect `log_name`, `log_type`, and `params` without a real device.
...
- [`@apps-in-toss/web-framework`](https://www.npmjs.com/package/@apps-in-toss/web-framework) — SDK package. `eventLog` is re-exported from [`@apps-in-toss/webview-bridge`](https://www.npmjs.com/package/@apps-in-toss/webview-bridge).
