...
id: deeplink-routing
title: deep-link parameter parsing and routing
sidebar_label: deep-link parameter routing
...
# deep-link parameter parsing and routing
...
Unknown paths (not in `ROUTES`) are silently ignored — the default entry screen remains. Centralising the route map means adding a new deep-link only requires one table update.
