...
Requires the `clipboard` permission. The call fails only when the status is **explicitly `denied`** — `notDetermined` lets the call through, but prompting the dialog first is the safer flow. Wrap in `try/catch`.
